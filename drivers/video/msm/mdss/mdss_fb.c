...
#include <linux/gpio.h>  //20130815 tracy add for ATS LCM test
...
#include "mdss_fb.h"
//20131018 tracy add for fix white screen when power on in 1030 SW++
...
static bool LCD_init=false;  // 20131125 tracy add for white screen after "rebot screen"
...
//20130815 tracy add for ATS LCM ID test++
			     ...
			     int op_enable)
{
	...
	default:
		if (mfd->panel_power_on && mfd->mdp.off_fnc) {
			int curr_pwr_state;
			//20131226 tracy add test log for MTBC stuck++
...
static int mdss_fb_release_all(struct fb_info *info, bool release_all)
{
	...
	if (!mfd->ref_cnt) {
		//20131125 tract add for fix white screen when shutdown++
