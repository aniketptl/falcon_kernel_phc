...
//20131118 tracy add for esd test and fix crash when phone sleep++
...
static int dsi_panel_handler(struct mdss_panel_data *pdata, int enable)
{
	...
	panel_info_local = pdata; 		//20131118  tracy add for esd test and fix crash when phone sleep
	if (enable) {
...
static void dsi_esd_wq_workqueue_handler(struct work_struct *work)
{
	...
	dsi_set_tx_power_mode(0);
		rc = dsi_cmds_tx_v2(panel_info_local, &dsi_panel_tx_buf,
