...
//[Arima Edison] modify it to 3 sec++
...
static int msm_otg_notify_power_supply(struct msm_otg *motg, unsigned mA)
{
	...
	else if (motg->cur_power >= 0 && (mA == 0 || mA == 2))  //[Arima Patch] 20140110 disable D+ D- charging by Abner
	{			
		/* Disable charging */
...
static void msm_otg_notify_charger(struct msm_otg *motg, unsigned mA)
{
	...
	if (motg->cur_power == mA && mA!=0) //[Arima Patch] 20140110 disable D+ D- charging by Abner
		return;
...
static void msm_otg_chg_check_timer_func(unsigned long data)
{
	...
	//[Arima Patch] 20140110 disable D+ D- charging by Abner
	if(0)
	{
		if(otg->phy->state == OTG_STATE_B_PERIPHERAL)
		{
				  ...
				  const union power_supply_propval *val)
{
	...
	//[Arima Edison] add for safety timer++	
	//case POWER_SUPPLY_PROP_TYPE:
		//psy->chg_type = val->intval;
		//printk(KERN_EMERG "POWER_SUPPLY_PROP_TYPE : %d\n",val->intval);
		//break;
	//[Arima Edison] add for safety timer--
	case POWER_SUPPLY_PROP_HEALTH:
