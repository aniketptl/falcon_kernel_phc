...
extern int maintenance_soc_flag;   //Abner 20131223
...
static int recalculate_raw_soc(struct qpnp_bms_chip *chip)
{
	...
	if (chip->use_voltage_soc) {
		soc = calculate_soc_from_voltage(chip);               //Abner 20131223
	  pr_debug("maintenance soc = %d,\n", soc);
	} else {
...
static int set_battery_data(struct qpnp_bms_chip *chip)
{
	...
	//Arima add battery calibration 20130923++	
